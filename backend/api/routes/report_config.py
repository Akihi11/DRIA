...
重要说明：
0. 【⚠️⚠️⚠️ 最高优先级规则 - 确认配置 vs 参数修改的严格区分 - 必须严格遵守】如果用户输入包含任何参数修改关键词（如"修改"、"改为"、"改成"、"通道"、"阈值"、"np"、"ng"、数字等），绝对不能解析为"确认配置"或"确认"等导航操作！必须解析为参数修改操作！只有当输入是纯粹的确认词汇（如"确认"、"确认配置"、"完成"、"好了"、"ok"等）且不包含任何参数修改关键词时，才能返回"确认配置"等导航操作。例如：用户说"修改通道为np，阈值200" -> 必须返回{{"actions": [{{"action": "修改监控通道", "value": "Np"}}, {{"action": "修改阈值", "value": 200}}]}}，绝对不能返回{{"action": "确认配置", "value": null}}！这是最高优先级规则，违反此规则会导致严重的解析错误！
0.5. 【⚠️⚠️⚠️ 最高优先级规则 - 条件名称优先级 - 必须严格遵守】如果用户明确说了"条件一"或"条件二"（例如："条件一的..."、"条件二的..."、"把条件一..."、"修改条件二..."等），必须无条件按照用户明确说的条件返回，绝对不能受当前上下文（current_condition）影响！例如：如果当前上下文是条件二，但用户明确说"条件一的阈值改为2000"，必须返回{{"action": "修改条件一阈值", "value": 2000}}，绝对不能返回"修改条件二..."！用户明确说的条件优先级永远最高！
...
【⚠️⚠️⚠️ 最高优先级规则 - 确认配置 vs 参数修改的严格区分】：
1. 【绝对禁止】如果用户输入包含任何参数修改关键词（如"修改"、"改为"、"改成"、"设置为"、"设置成"、"通道"、"阈值"、"统计方法"、"持续时长"、"判断依据"、"np"、"ng"、"最大值"、"最小值"、"平均值"等），绝对不能解析为"确认配置"或"确认"等导航操作！必须解析为参数修改操作！
2. 【正确示例】：
   - 用户说"修改通道为np，阈值200" -> 必须返回{{"actions": [{{"action": "修改监控通道", "value": "Np"}}, {{"action": "修改阈值", "value": 200}}]}}，绝对不能返回{{"action": "确认配置", "value": null}}！
   - 用户说"通道np，阈值200" -> 必须返回{{"actions": [{{"action": "修改监控通道", "value": "Np"}}, {{"action": "修改阈值", "value": 200}}]}}，绝对不能返回{{"action": "确认配置", "value": null}}！
   - 用户说"通道改为np，阈值改为200" -> 必须返回{{"actions": [{{"action": "修改监控通道", "value": "Np"}}, {{"action": "修改阈值", "value": 200}}]}}，绝对不能返回{{"action": "确认配置", "value": null}}！
3. 【错误示例 - 绝对禁止】：
   - 用户说"修改通道为np，阈值200" -> 错误：{{"action": "确认配置", "value": null}} ❌（这是完全错误的！）
   - 用户说"通道np，阈值200" -> 错误：{{"action": "确认配置", "value": null}} ❌（这是完全错误的！）
4. 【确认配置的正确识别】：
   - 只有当用户输入是纯粹的确认词汇（如"确认"、"确认配置"、"完成"、"好了"、"ok"等），且不包含任何参数修改关键词时，才能返回"确认配置"等导航操作。
   - 如果用户输入同时包含确认词汇和参数修改关键词，必须优先解析为参数修改操作，忽略确认词汇！
   - 例如：用户说"确认，修改通道为np" -> 必须返回{{"action": "修改监控通道", "value": "Np"}}，不能返回{{"action": "确认配置", "value": null}}！
5. 【参数修改关键词列表】如果输入包含以下任何关键词，必须解析为参数修改，不能解析为确认配置：
   - 修改类动词："修改"、"改为"、"改成"、"设置为"、"设置成"、"改成"、"改为"、"改成"、"设为"、"设成"
   - 参数名称："通道"、"监控通道"、"阈值"、"统计方法"、"持续时长"、"判断依据"、"判据"、"逻辑"、"高阈值"、"低阈值"、"T1"、"T2"
   - 参数值："np"、"ng"、"最大值"、"最小值"、"平均值"、"中位数"、"瞬时值"、"大于"、"小于"、"大于等于"、"小于等于"、数字（如"200"、"1500"等）
6. 【⚠️重要】如果用户只说"下一步"、"继续"、"确认"、"完成"等导航操作（且不包含任何参数修改关键词），必须返回对应的action，绝对不能解析为参数修改操作（如"修改监控通道"）！例如，如果用户说"下一步"，必须返回{{"action": "下一步", "value": null}}，绝对不能返回{{"action": "修改监控通道", "value": "Np"}}等参数修改操作！
...
请返回JSON格式："""
    ...
    try:
        ...
        async with LLMClient(config) as client:
            messages = [
                Message(role="system", content="你是报表参数助手，请理解用户自然语言需求，转成可自动调用后端配置指令结构。你返回的JSON必须严格符合格式要求。\n\n【⚠️⚠️⚠️ 最高优先级规则】如果用户输入包含任何参数修改关键词（如\"修改\"、\"改为\"、\"通道\"、\"阈值\"、\"np\"、\"ng\"、数字等），绝对不能解析为\"确认配置\"！必须解析为参数修改操作！只有当输入是纯粹的确认词汇（如\"确认\"、\"完成\"等）且不包含任何参数修改关键词时，才能返回\"确认配置\"。"),
